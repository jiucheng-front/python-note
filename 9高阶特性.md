...
一类是集合数据类型，如`list`、`tuple`、`dict`、`set`、`2字符串拼接.py`等；
...
生成器都是`Iterator`对象，但`list`、`dict`、`2字符串拼接.py`虽然是`Iterable`，却不是`Iterator`。

把`list`、`dict`、`2字符串拼接.py`等`Iterable`变成`Iterator`可以使用`iter()`函数：
...
为什么`list`、`dict`、`2字符串拼接.py`等数据类型不是`Iterator`？
